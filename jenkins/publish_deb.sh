...
mkdir -p $PKG_DIR
cd $PKG_DIR
git clone /var/local/jenkins/bazel/pkgbuild
cd $PKG_DIR/pkgbuild/bazel

# The bazel repository is mounted into our Docker image, but read-only.
# We clone into a directory inside Docker (this is faster than cp).
git clone /var/local/jenkins/bazel
...
$PKG_DIR/pkgbuild/bazel/pkgbuild_jenkins.sh

ls -l *.deb

# The directory that is copied from Docker back into the Jenkins workspace.
COPY_FROM_DOCKER=/var/local/git/bazel/testoutput
